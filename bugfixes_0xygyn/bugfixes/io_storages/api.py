...
class ImportStorageDetailAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        # Vulnerable: no explicit organization permission check
        # if not permission_org(self.request.user, obj.project):
        #     raise PermissionDenied
        # Fix: ensure user has permission on the related project
        if not permission_org(self.request.user, obj.project):  # issues #97, #95, #94, #93
...
class ExportStorageDetailAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        obj = super().get_object()
        # Vulnerable: missing permission check allowed IDOR access
        # Fix: verify organization permissions before returning object
        if not permission_org(self.request.user, obj.project):  # issues #97, #95, #94, #93
            raise PermissionDenied("User lacks organization permissions")
        return obj
